-- =============================================================================
-- Oracle SAM Discovery Script — JSON output
-- Run with SQL*Plus directly on the Oracle DB server when Ansible is unavailable.
--
-- Usage:
--   sqlplus / as sysdba @oracle_discovery_json.sql
--   sqlplus sam_discovery/<password>@<tns_alias> @oracle_discovery_json.sql
--
-- Output: oracle_discovery_<hostname>_<date>.json
--   Feed this file to scripts/load_discovery.py to import into the SAM database.
--
-- Feature usage is collected from DBA_FEATURE_USAGE_STATISTICS (preferred over
-- v$option) because it records actual detected usage counts, first/last used
-- dates, and current usage state — the same data Oracle auditors examine.
-- =============================================================================
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('"_meta":{"script_version":"2.0","generated":"' || TO_CHAR(SYSDATE,'YYYY-MM-DD"T"HH24:MI:SS') || '"},');
  ...
  -- Feature usage replaces the old "options" block
