...
BEGIN
    ...
    BEGIN
      ...
      IF EXISTS (SELECT 1 FROM %I.oracle_processors WHERE server_id = v_server_id) THEN
        UPDATE %I.oracle_processors SET
          ...
          virt_type        = (COALESCE(p_payload->>'virt_type','unknown'))::%I.virt_type,
      ...
      LOOP
        CONTINUE WHEN v_instance->>'sid' IS NULL OR TRIM(v_instance->>'sid') = '';
  ...
  p_schema,   -- 3: environment_type enum
  p_schema,   -- 4: oracle_processors (EXISTS check)
  p_schema,   -- 5: oracle_processors (UPDATE)
